...
library(xgboost)
target_train<- F_train$loan_status
target_test <- F_test$loan_status
F_train[,loan_status := NULL]
F_test[,loan_status := NULL]
dtrain<-xgb.DMatrix(data=data.matrix(F_train),label=data.matrix(target_train),missing=NA)
dtest<-xgb.DMatrix(data=data.matrix(F_test),label=data.matrix(target_test),missing=NA)
#XGB
#150 0.971 depth 7
#250 0.972 ''
#500 0.974

model_xgb <- xgboost(data = dtrain, label=as.matrix(target_train),nrounds = 1100, eval_metric="auc",
                     max_depth = 8, eta = 0.01,subsample=0.6, colsample_bytree=0.8, objective = "binary:logistic")
pred <- predict(model_xgb, dtest)
mysolution = data.frame( member_id = test$member_id, loan_status = pred, stringsAsFactors = FALSE)
submission_xgb = mysolution
write.csv(submission_xgb, file = "xgb5.csv", row.names = FALSE)
...
# Training a Deep learing Model ----------------------------------------------------

deep_clf <- h2o.deeplearning(x = x
                   ,y = y
                   ,training_frame = h2o_train
                   ,validation_frame = split_val
                   ,ignore_const_cols = T
...
deep_clf #Validation Accuracy = 0.9858

deep_clf_pred <- as.data.table(h2o.predict(gbm_clf_3,h2o_test))
head(gbm_clf_pred,10)
...
##Xgboost Model
library(caret)
x <- createDataPartition(y = train$loan_status,p = 0.70,list = F)
dtrain <- train[x,]
dval <- train[-x,]
t1 <- dtrain$loan_status
t2 <- dval$loan_status
t3 <- test$loan_status
dtrain$loan_status=NULL
dval$loan_status=NULL
test$loan_status=NULL
dtrain <- xgb.DMatrix(data = as.matrix(dtrain),label=as.matrix(t1))
dval <- xgb.DMatrix(data = as.matrix(dval),label=as.matrix(t2))
dtest <- xgb.DMatrix(data = as.matrix(test),label=as.matrix(t3))
...
bst1 <- xgb.train(params = xgb_params,
                  data = dtrain,
                  nrounds = 800, #set num_rounds from cv
                  watchlist = watchlist,
                  eval_metric="auc",
                  print_every_n = 10,
                  verbose = 1,
